...
class TestIPC(TestPHYBase):
    def test_send_data_between_transceivers(self):
        alice = MockTransceiver(name='Alice')
        bob = MockTransceiver(name='Bob')

        medium = MockMedium(name='air')

        alice.connect(medium)
        bob.connect(medium)

        alice.transmit(alice_tx_data := 0)

        # There might be some unknown error that occurs. To avoid the test blocking
        # forever, we'll wait a small amount of time and fail if we don't get anything.
        if not bob.osi_listener.poll(timeout=0.1):
            self.fail('No data in alice to bob test')

        self.assertEqual(bob.osi_listener.recv(), alice_tx_data)

        bob.transmit(bob_tx_data := 1)

        if not alice.osi_listener.poll(timeout=0.1):
            self.fail('No data in bob to alice test')

        self.assertEqual(alice.osi_listener.recv(), bob_tx_data)
